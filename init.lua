...
do
  ...
  for _, mode in ipairs(modes) do
    vim.keymap.set(mode, '<left>', '<cmd>echo "Use h to move!!"<CR>')
    vim.keymap.set(mode, '<right>', '<cmd>echo "Use l to move!!"<CR>')
    vim.keymap.set(mode, '<up>', '<cmd>echo "Use k to move!!"<CR>')
    vim.keymap.set(mode, '<down>', '<cmd>echo "Use j to move!!"<CR>')
  end
...
do
  ...
  -- Auto-close pairs: type `(` and get `()` with your cursor in between,
  -- same for `{`, `[`, `"`, `'`, backtick. `<BS>` inside an empty pair
  -- deletes both, and typing the closer just moves past it instead of
...
do
  ...
  local function start_jdtls()
    ...
    -- Lombok: jdt.ls doesn't run annotation processors itself, so members
    -- generated by @Getter/@Setter/@Data/@Builder are invisible to it and
    -- show up as bogus "cannot resolve method getFoo()" errors. Loading
    -- Lombok as a Java agent into jdt.ls's own JVM makes it expand those
    -- annotations during analysis. Mason ships a lombok.jar inside the jdtls
    -- package, so no extra download is needed.
    ...
    if vim.uv.fs_stat(lombok_jar) then
      -- `--jvm-arg=` is understood by the Mason `jdtls` launcher, which
      -- forwards the flag to the JVM running jdt.ls. Insert it before `-data`.
    ...
    local config = {
      ...
      on_attach = function(_, bufnr)
        ...
        -- Wipe this project's jdt.ls workspace cache and restart the server.
        -- jdt.ls caches its compiled index per project; after a dependency
        -- change (adding Lombok, bumping versions) that cache can go stale and
        -- keep reporting phantom errors. This force-kills the server (so it
        -- can't flush the old cache back on a clean shutdown), deletes only the
        -- regenerable workspace dir, then re-indexes from source.
        map('<leader>jw', function()
          -- Sanity guard: never delete anything outside the jdtls-workspace tree.
          if not workspace_dir:find('/jdtls%-workspace/', 1) then
            vim.notify('jdtls: refusing to delete suspicious path ' .. workspace_dir, vim.log.levels.ERROR)
            return
          end
          -- force = true: kill the process instead of a graceful shutdown, so
          -- it doesn't rewrite the stale index we're about to remove.
          vim.lsp.stop_client(vim.lsp.get_clients { name = 'jdtls' }, true)
          if vim.fn.delete(workspace_dir, 'rf') == 0 then
            vim.notify('jdtls: wiped ' .. workspace_dir .. ', re-indexing…', vim.log.levels.INFO)
          else
            vim.notify('jdtls: failed to delete ' .. workspace_dir, vim.log.levels.WARN)
          end
          -- Give the killed server a moment to release the dir before restart.
          vim.defer_fn(start_jdtls, 500)
        end, '[W]ipe workspace & restart')
